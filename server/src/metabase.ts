import { getEnvSync } from "./config.js";

async function agentRequest(path: string, options?: RequestInit) {
  const url = `${getEnvSync("METABASE_INSTANCE_URL")}/api/agent${path}`;

  const response = await fetch(url, {
    ...options,
    headers: {
      "X-API-Key": getEnvSync("METABASE_API_KEY"),
  ...
  if (!response.ok) {
    console.log(`Metabase API ${response.status}: ${body}`);
